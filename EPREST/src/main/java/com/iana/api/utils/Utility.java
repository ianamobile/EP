...
public class Utility {
	...
	/*
	 * returns SQL date based on String date and format passed added by ashok
	 * 12/06/06
	 */
	public static java.sql.Date stringToSqlDate(String p_string, String p_format) {
		...
		try {
			date = df.parse(p_string);
			/*
			 * log.debug("dda3 .. " + p_format); log.debug("dda4 .. " + p_string);
			 * log.debug("dda .. " + date); log.debug("dda1 .. " + date.getTime());
	...
	/*
	 * added by ashok for converting String to double (The string can be comma
	 * separted also) The commas will be removed and then StringtoDouble will be
	 * called
	 */
	...
	/*
	 * To convert the String into Long Amount this fun will check for M .. If M,
	 * then to consider Million else for normal amount to remove comma and then
	...
	public static long amountConversion(String p_amount) {
		...
		/*
		 * Checking whether decimal present or not and if present, then to remove the
		 * same
		 */
	...
	public static String getPassword() {
		...
		for (int i = 0; i < PASSWORDLENGTH; i++) {
			...
			switch (rndm) {
			case 0:
				tempChar = '0' + (int) (Math.random() * 10);
				break;
			case 1:
				tempChar = 'a' + (int) (Math.random() * 26);
				break;
			case 2:
				tempChar = 'A' + (int) (Math.random() * 26);
				break;
	...
	/*
	 * Added by Abhinav for converting Limits and Deductibles into comma separted
	 * string
	 **/
	public static String createCommaString(String value) {
		String str1 = ",";
		...
		if (myNum.indexOf('-', 0) > -1) {
			myNum = myNum.substring(1, length);
			value = myNum;
			flag = 1;
		}
		if (value == null || value.equals("")) {
		...
		if (myNum.indexOf('.', 0) > -1) {
			...
			value = myNum.substring(0, myNum.indexOf('.', 0));
		}
		double n = Math.floor(Double.parseDouble(value));
		...
		while (n > 0) {
			...
			if (var != 0) {
				...
				int z = len;
				while (z > 3) {
					String x = value.substring(z - 3, z);
					z = z - 3;
					arr[i] = x + "";
					i++;
				}
				arr[i] = value.substring(0, z);
				int k = i;
				while (k >= 0) {
					dedValue = dedValue + arr[k];
					if (k > 0)
						dedValue = dedValue + str1;
